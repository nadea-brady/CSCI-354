...
function fetchMonstersData() {
    const docRef = doc(db, "monsters", "monsters");
    getDoc(docRef)
        .then((docSnap) => {
            if (docSnap.exists()) {
                const data = docSnap.data();
                const monsterTypes = Object.keys(data); // Get monster types
                createMonsterTypeButtons(data, monsterTypes); // Process the data
            } else {
...
// The rest of your existing functions (createMonsterTypeButtons, createMonsterButtons, etc.) remain unchanged
...
function createMonsterTypeButtons(monstersData, monsterTypes) {
    ...
    monsterTypes.forEach(type => {
        ...
        button.textContent = type;  // e.g., "Flying Wyverns"
